...
-- Add new columns for shared sales if they don't exist
ALTER TABLE public.sales
ADD COLUMN IF NOT EXISTS shared_with_id uuid REFERENCES auth.users(id),
ADD COLUMN IF NOT EXISTS shared_with_email text;

-- Migrate existing data if shared_with column exists
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'sales' AND column_name = 'shared_with') THEN
    UPDATE public.sales
    ...
    -- Drop old column
    ALTER TABLE public.sales DROP COLUMN shared_with;
  END IF;
END $$;
